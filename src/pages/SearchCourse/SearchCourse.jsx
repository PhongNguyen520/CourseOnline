...
export default function SearchCourse() {
  ...
  const getBookmark = async () => {
    try {
      const response = await requests.get(BOOKMARK_URL);
  ...
  const filteredCourses = resultCourse
    .filter((course) => {
      ...
      if (priceRange === "50-100" && (course.price < 50 || course.price > 100))
        return false;
      if (priceRange === ">100" && course.price <= 100) return false;
      return true;
    })
    .filter((course) => {
      if (rating !== "all" && course.averageStarRating < parseInt(rating))
        return false;
      return true;
    })
    .filter((course) => {
    ...
    const addBookMark = async (courseId) => {
        try {
          ...
          await requests.post(BOOKMARK_URL, params, {
            headers: {
            ...
            <Row className="justify-content-between align-items-center mt-4">
              <Col md={6}>
                <div className={cx("sort-buttons")}>
                  <Button
                    variant={
                      sortOrder === "newest" ? "primary" : "outline-primary"
                    }
                  ...
                  <Button
                    variant={
                      sortOrder === "lowest" ? "primary" : "outline-primary"
                    }
                  ...
                  <Button
                    variant={
                      sortOrder === "highest" ? "primary" : "outline-primary"
                    }
                ...
                <Row className={cx("course-items")}>
                  {filteredCourses.map((course, index) => {
                    const isBookmarked = bookmarkedCourses.some(
                      (bookmark) => bookmark.course.courseId === course.courseId
                    );
                    return (
                      <Col lg="3" key={index} className={cx("course-item")}>
                        <div className={cx("course-image")}>
                          <span
                            ...
                            onClick={() =>
                              !isBookmarked && addBookMark(course.courseId)
                            }
                          >
                            <i
                              ...
                              style={{
                                fontSize: "24px", 
                                color: isBookmarked ? "red" : "#91CCF7", 
                              }}
                            />
                          </span>
                          <img
                            src={course.image || images.courseDefault}
